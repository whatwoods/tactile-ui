...
* 可参考的官方材料是列表/设置项的白色承载面、`MenuDialog` / `ProgressDialog` 的 9-patch 面板，以及 OneStep/BigBang 的场景化容器。
...
* 官方 xxhdpi 资源尺寸：`switch_ex_mask/frame = 224 × 144px`，折算约 `74.7 × 48dp`；`onMeasure` 额外在 Y 轴加入 `2dp × 2` 扩展点击区域，因此 Web 中 switch 外盒约为 `75 × 52px`。
* `switch_ex_unpressed = 338 × 144px`，滑块实际可见区域约 `32 × 36dp`；Web 中以 `36px` 凸起滑块近似表达。
* 开关位移来自 `sBtnOnPos = sBtnWidth / 2` 与 `sBtnOffPos = sMaskWidth - sBtnWidth / 2`，Web 中沉淀为约 `39px` 的滑块 travel。
* 滑块按 60fps 推进，速度约 `350dp/s`，拖拽和点击都可触发切换。
* 禁用态 alpha 为 0.75。
* 拖拽时根据滑块中心是否越过中点决定最终状态；轻触仍走直接 toggle。
* Web 组件不需要照搬 Java 位图合成，但必须保留：可拖拽/可点击、键盘可切换、短促位移、凸起滑块、轨道凹槽、禁用仍可辨识。
...
* Slider 不应复用 `SwitchEx` 的滑块尺寸。`SwitchEx` 已校准为约 `36px` 滑块，而 Slider 旋钮继续使用约 `30px`，避免设置页里两类控件层级混淆。
* 当前稳定结构 token：控制区高度 `34px`，轨道高 `8px`，轨道横向 padding `16px`，节点 `16px`，节点边框 `4px`，旋钮 `30px`，旋钮中心凹点 `12px`。
...
`Chip` 用于少量短文本的即时选择或文本块操作。它不是官方独立组件，应按按钮资源和 BigBang 文本块场景推导。

官方校准状态：

* 当前 SmartisanOS base 仓库未发现独立通用 Chip/Pill 组件。
* 可参考的官方机制是按钮 9-patch 的凸起/按压状态、`search_btn.9.png` 的短按钮资源，以及 BigBang 文本分块后的短文本操作场景。
* 当前 Web `Chip` 属于推导组件，主要用于 demo 中的文本块、标签和短操作，不应替代 `Button`、`SegmentedControl` 或 check 型列表。
* Chip 文案应短，适合 1-8 个字符；长标签应使用按钮、列表项或 Select。

结构规则：

* 默认态使用实体按钮质感，选中/按下态可以使用系统蓝色高亮。
* 逗号、句号等极短文本可使用 compact 尺寸，但仍要保持可点击面积。
* 多个 Chip 的互斥选择如果承担主要设置语义，应升级为 `SegmentedControl` 或 check 型列表。
...
`Select` 是 Web 表单里的轻量下拉选择控件，不应伪装成 SmartisanOS 官方 `MenuDialog` 或 BigBang 搜索选项弹窗。

官方校准状态：

* 当前 SmartisanOS base 仓库未发现独立通用 Spinner/Select 视觉资源。
* 已确认存在的是底部 `MenuDialog`、`menu_dialog_list_item.xml`、`menu_dialog_list_multi_item.xml`、`MenuDialogListAdapter`、`MenuDialogMultiAdapter`，以及搜索/BigBang/OneStep 场景内的专用 popup。
* `MenuDialog` 是底部贴边、强遮罩、全宽决策菜单；它不是输入框旁的小型下拉。
* BigBang 搜索选项弹窗是文本爆炸后的即时动作来源选择；它不应替代普通表单 `Select`。
...
## 13.16 DragBubble / FloatText 拖拽气泡
...
## 13.17 OneStepItem / OneStepPanel 内容列表
...
## 13.18 OneStepTopBar / Shortcut / PhotoGrid 侧栏入口

这些组件只服务 OneStep 场景，不是通用导航、通用图标按钮或通用图片网格。

官方校准来源：

* `packages_apps_OneStep/res/layout/topbar_view.xml`
* `packages_apps_OneStep/res/layout/top_item_view.xml`
* `packages_apps_OneStep/res/layout/app_item.xml`
* `packages_apps_OneStep/res/layout/photo_line_sub_view.xml`
* `sidebar_width = 48dp`
* `recent_photo_size = 80dp`
* `recent_photo_padding_vertical = 5dp`
* `recent_photo_padding_bottom = 10dp`
* `topbar_photo_icon_content_size = 33dp`
* `topbar_photo_icon_content_paddingTop = 14dp`
* `topbar_file_icon_content_paddingTop = 10dp`
* `top_view_space_left/right = 3dp`
* `sidebar_list_item_padding_top = 3dp`

结构规则：

* `OneStepTopBar` 固定为横向 3 个入口，两侧各有 `3dp` dim space，底部和左右有 `4dp` 阴影边。
* TopBar 单项图标画布为 `45 × 53dp`，图标内容约 `33dp`，文字字号 `11dp`，文字最大宽 `50dp`，箭头为 `9 × 15dp`。
* `OneStepShortcut` 使用 `48dp` 侧栏槽位；普通 App/联系人图标内容为 `32dp`，上下 padding 为 `3dp`。`previous_app_icon` 是特殊全槽位入口，不应反推普通快捷方式尺寸。
* `OneStepPhotoGrid` 每行最多 3 张，单张 `80dp`，左右间距 `5dp`，底部间距 `10dp`；失败、打开图库、更多等状态必须保留图片历史语义。
* 这些组件不得替代普通 `Header`、`SideNav`、`Button`、`Icon`、`Card` 或通用图片网格。

---

## 13.19 BigBangOptionPopup 搜索选项弹窗
...
* BigBang 文本爆炸后的普通文本块由 `Chip` 的推导规则承接；不要继续新增更多 BigBang 专用“短标签按钮”组件。
...
* 顶部/底部 chrome、返回、浏览器、设置和搜索来源按钮均为 BigBang 场景资源，不应反推到通用 `Dialog`、`Card` 或 `SegmentedControl`。
...
* 当图标承担官方资源语义，如列表箭头、check 标记、OneStep 文件图标时，应优先使用对应组件的专用 token，而不是直接套通用 Icon。
...
## 13.22 Demo Shell / Header / SideNav 展示壳层
...
* `SideNav` 是 Web 文档导航，不是 OneStep 侧栏；OneStep 侧栏相关规则只适用于 `OneStepTopBar`、`OneStepShortcut`、`OneStepItem` 等场景组件。
