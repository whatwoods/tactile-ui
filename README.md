...
```
请直接阅读 [AGENTS.md](https://github.com/whatwoods/tactile-ui/blob/main/AGENTS.md) 获取完整的使用规则和最佳实践。
```
...
│   ├── main.tsx               # 应用入口
│   ├── components/            # Tactile UI 公共组件库
│   │   ├── Button/            # 物理琴键按钮
│   │   ├── Switch/            # 立体拨码开关
│   │   ├── Slider/            # 物理阻尼滑块
│   │   └── ...                # 其他已校准拟物组件
│   ├── demo/
│   │   └── components/        # 仅用于站点演示的页面区块
│   └── styles/
...
```bash
npm run lint       # ESLint + design-token hardcoding guardrail
npm run test       # component behavior tests
npm run build      # library build plus demo build
...
构建后的组件库包名为 `tactile-ui`。在消费项目中引入全局样式一次，再从包入口导入组件：

```tsx
import React, { useState } from 'react';
import { Card, SettingItem, Button } from 'tactile-ui';
...
公共组件 API、状态、键盘行为和可访问性说明见
[docs/COMPONENTS.md](./docs/COMPONENTS.md)。键盘与 ARIA 矩阵见
